...
function setCacheHeaders(res, maxAgeSeconds, staleWhileRevalidateSeconds) {
    res.set({
        'Cache-Control': `public, max-age=${maxAgeSeconds}, stale-while-revalidate=${staleWhileRevalidateSeconds}`,
        'ETag': `"${Date.now()}"`,
    });
}
function handleRequest(_req, res, data, cacheConfig) {
...
contentRouter.get('/bundle', (_req, res) => {
...
contentRouter.get('/experience', (_req, res) => {
